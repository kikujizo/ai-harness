...
| 対象 | 判断 | 根拠 |
|---|---|---|
| `mino-context-discovery` | labで残す | GitHub上の実行例1件で、意味の断層と保留事項を素材へ遡って整理できた |
| `mino-contract-driven-coding` | labで残し、実行検証する | 仕様上書き防止の設計ガードはレビューで観測。コード生成効果は未確認 |
| その他4 Skill | labで残し、単独検証する | 導入・静的レビューのみで実行効果は未確認 |
| Skill間連携 | 自動化せず、明示指定を維持 | PR #42で幻参照と自動連鎖の危険を確認 |
