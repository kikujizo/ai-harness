...
| Codex/Cursor/ChatGPTが行動不能・停滞 | Claude Code（例外委譲） | Codex ＋ ChatGPT（不可なら人間） |
| 原因不明・複雑設計・緊急復旧（PM/人間が例外委譲） | Claude Code（例外委譲） | Codex ＋ ChatGPT（不可なら人間） |
...
高リスク時の PM verdict 推奨: `PM_VERDICT: approve risk=high gate=human_approval`。
人間は承認判断者であり作業者ではない。承認後は人間が実装担当を明示指定し、`route=cursor` 等を付与する。
