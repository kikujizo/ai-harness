...
判断を人間に仰ぐときの形式を固定する（人間は判断・承認専任PMであり、オープンな問い返しは判断コストの転嫁になる）:

- オープンクエスチョン（「どうしますか？」「〜でしょうか？」）で閉じない。
  「次の候補から選んでください」「ご指定ください」「この案の承認可否をください」のようにクローズドに閉じる
- 相談はすべて**提案**として行い、推奨を必ず付ける
- 推奨・非推奨には必ず理由を添える（端的でよいが、人間が選ぶための判断材料になっていること）
- 選択肢は必須ではない。提案が1つなら無理に複数案を作らず、承認/否認で答えられる形で示す
- 選択肢を箇条書きで提示した後、独立した段落で別の質問を重ねない（1回の問いかけ＝1つの判断）
