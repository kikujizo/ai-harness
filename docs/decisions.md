...
# Decision: mino-socratic-requirementsの現行方式を見送る
...
`mino-socratic-requirements`の実験記録は残すが、自由記述を①から積み上げる現行方式は実務で再利用しない。Issue #46の初回比較では、10問の問いがすべて最終成果物の要素へ使われ、人間が承認する具体的な問題文へ到達した。

一方、10問＋確認往復の負担は具体化効果に見合わないと人間が判断した。Skill固有の質問選択が通常の対話型仕様化より優れるかは**未確認**である。少数質問でAIが推論・提案し、人間が選択肢から答える再設計は候補に残すが、core昇格・自動発動は行わない。
...
| 一括baselineより人間の回答負担が大きい | **観測済み** | 追加回答0回に対し10問＋確認往復 |
...
- 背景と目的を先に説明する
- AIが既知情報から推論した案を先に提示する
- 人間への質問を少数に絞り、原則として選択式にする
- 実行開始時刻、終了時刻、質問数、確認往復を記録する

本IssueではSkill本体を変更しない。修正する場合は別Issueで判断する。

### 追加検証

- 現状では優先しない
- 上記の再設計案が採用された場合だけ、現行方式より質問・確認回数を減らせるか比較する

### 見送る

- core昇格
- 一般依頼からの自動発動
- 自由記述を①から積み上げる現行方式の実務再利用
- 1件だけを根拠にした一般的優位性の主張

## 比較上の限界

- baselineは追加質問なし、Skill条件は10問のため、追加情報量の効果を除去できない
- ソクラテス条件の開始時刻を記録しておらず、所要時間を比較できない
...
- [x] 人間が再利用意向を回答する（現行方式は再利用しない。少数質問・AI推論・選択式なら再設計候補）
- [ ] ChatGPT要件レビュー
- [ ] 独立技術レビュー
- [ ] 人間merge判断
