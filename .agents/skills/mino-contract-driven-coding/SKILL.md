...
- 根拠のない数値を作らない。
- 範囲外の挙動も明記する。既定は「拒否（エラー）」だが、丸め・黙認を選ぶなら承認済み根拠が必要。

**承認ゲートG1**: 契約表を提示し、上位仕様から導出されたことを確認する。新制約がある場合はここで停止する。
...
2. **公開インターフェイス定義**: 目的駆動の名前。実装前に提示する。
3. **実装コード**: 実装権限がある担当のみ生成する。ChatGPT/Codexはここを「生成不可」と書く。
4. **契約検証テスト**: 契約表の行と対応するテスト。
5. **突合表＋判断根拠・確認事項・保留事項**: 契約表行／実装箇所／テスト箇所、未担保行、確認事項、保留事項。
